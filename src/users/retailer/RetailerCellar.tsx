...
const RetailerCellar: React.FC = () => {
  ...
  const [squareItems, setSquareItems] = useState<RetailerInventory[] | null>(
    null
  );
  const [squareIsLoading, setSquareIsLoading] = useState(false);
  const [addedFromSquare, setAddedFromSquare] = useState<Set<string>>(
    new Set()
  );
  ...
  useEffect(() => {
    ...
    const all = mockInventory.filter(
  ...
  const handleAddFromSquare = (index: number) => {
    ...
    setSelectedSquare((prev) => {
      ...
      if (next.has(item.externalItemId)) next.delete(item.externalItemId);
  ...
  const handleBatchPersist = () => {
    if (!squareItems) return;
    const invIds = new Set(inventory.map((i) => i.externalItemId));
    const toPersist = squareItems.filter(
      (i) =>
        selectedSquare.has(i.externalItemId) &&
        !addedFromSquare.has(i.externalItemId) &&
        !invIds.has(i.externalItemId)
    );
    ...
    alert(
      `Persisted ${toPersist.length} item(s) and sent for WineCanon matching.`
    );
    ...
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p>
            Current Inventory shown below. Fetch from Square to review and add
            new wines.
          </p>
        </div>
      </div>
                                ...
                                {(() => {
                                  ...
                                  return matchedWine ? (
                                    <span className="small text-muted">
                                      {matchedWine.name} ({matchedWine.vintage}){" "}
                                      {matchedWine.varietal}
                                    </span>
                                  ) : null;
                                })()}
                              </div>
                    ...
                    <td>
                      {isEditing ? (
                        <input
                          ...
                          type="number"
                          onChange={(e) =>
                            handleEditChange(index, "varietal", e.target.value)
                          }
                        />
                      ) : (
                        item.varietal
                    ...
                    <td>
                      {isEditing ? (
                        <input
                          ...
                          value={item.producer}
                          onChange={(e) =>
                            handleEditChange(index, "producer", e.target.value)
                          }
                        />
                      ) : (
                        item.producer
      ...
      <div className="card mt-4">
        <div className="card-header d-flex justify-content-between align-items-center">
          ...
          <button
            ...
            disabled={squareIsLoading}
          >
            <i className="bi bi-arrow-repeat me-2"></i>
            {squareIsLoading ? "Fetching…" : "Fetch from Square"}
          </button>
        </div>
        <div className="card-body">
          {!squareItems && !squareIsLoading && (
            <div className="text-muted">
              Click "Fetch from Square" to load your POS items for review and
              ...
              <div className="d-flex justify-content-between align-items-center mb-2">
                ...
                <div>
                  <button
                    ...
                    disabled={
                      !squareItems || Array.from(selectedSquare).length === 0
                    }
                  >
                      ...
                      <th style={{ width: 40 }}>
                        <input
                          ...
                          checked={
                            ...
                            (() => {
                              const invIds = new Set(
                                inventory.map((i) => i.externalItemId)
                              );
                              const selectable = squareItems.filter(
                                (i) =>
                                  !addedFromSquare.has(i.externalItemId) &&
                                  !invIds.has(i.externalItemId)
                              );
                          ...
                          onChange={() => {
                            ...
                            const allSelectable = squareItems.filter(
                              (i) =>
                                !addedFromSquare.has(i.externalItemId) &&
                                !invIds.has(i.externalItemId)
                            );
                            const allSelected = allSelectable.every((i) =>
                              selectedSquare.has(i.externalItemId)
                            );
                            setSelectedSquare((prev) => {
                              const next = new Set(prev);
                              if (allSelected) {
                                allSelectable.forEach((i) =>
                                  next.delete(i.externalItemId)
                                );
                              } else {
                                allSelectable.forEach((i) =>
                                  next.add(i.externalItemId)
                                );
                              }
                              return next;
                            });
                      ...
                      <tr key={item.externalItemId}>
                        <td>
                          <input
                            className="form-check-input"
                            type="checkbox"
                            checked={selectedSquare.has(item.externalItemId)}
                            disabled={
                              addedFromSquare.has(item.externalItemId) ||
                              inventory.some(
                                (inv) =>
                                  inv.externalItemId === item.externalItemId
                              )
                            }
                            onChange={() => handleAddFromSquare(idx)}
                          />
                        </td>
                        <td>
                          <input
                            ...
                            value={item.name}
                            onChange={(e) =>
                              handleSquareEditChange(
                                idx,
                                "name",
                                e.target.value
                              )
                            }
                            readOnly={item.source !== "SQUARE"}
                          />
                        </td>
